...
<section class="hero-slider" style="min-height: 160px; position: relative;">
    <div class="container-fluid no-gutters p-0">
      ...
      " alt="apartment search banner" class="hero-bg-image"
                    ...
                    <div class="row g-3" style="width: 100%; margin: 0;">
                        ...
                        <div class="col-12 col-md-3 d-flex flex-column" style="min-width: 0;">
                            <input type="text" class="form-control flex-grow-1" id="search_name" name="name"
                                placeholder="Apartment name..." style="height: 45px;">
                        </div>
        ...
        <div class="col-lg-7 my-5">
            <div class="list-hotel h-100 d-flex flex-column">
                ...
                <div class="results-count d-flex align-items-center justify-content-between">
                    <div>
                        Found <b>{{ $apartments->total() }} Apartments</b>
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-4 col-md-6 col-sm-12 mb-4">
                        <div class="tour-item tour-item--grid rounded-4" data-plugin="matchHeight" data-id="{{ $apartment->id }}"
                            data-lat="{{ $apartment->lat }}" data-lng="{{ $apartment->long }}">
                            <div class="hotel-item__thumbnail position-relative">
                                ...
                                @endif
                                <a href="{{route('view-apartment', ['id' => $hashids->encode($apartment->id)])}}" style="display:block;">
                                    <img 
                                        src="{{ $apartment->cover_photo ? asset('storage/' . $apartment->cover_photo) : 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=360&h=240&fit=crop&crop=center' }}"
                                        alt="{{ $apartment->title }}"
                                        loading="eager"
                                        width="360"
                                        height="160"
                            ...
                            <div class="hotel-item__details" style="padding-top:18px;">
                                ...
                                <h3 class="hotel-item__title" style="font-size:1.25rem;font-weight:600;">
                                    <a href="{{route('view-apartment', ['id' => $hashids->encode($apartment->id)])}}" style="color:#222;text-decoration:none;">{{ $apartment->title }}</a>
                                </h3>
                                ...
                                <div class="hotel-item__price mb-3">
                                    <span class="_retail" style="color:#2e8b57;font-size:1.3rem;font-weight:600;">USD {{ number_format($apartment->base_price, 2) }}</span>
                                ...
                                <a class="btn btn-primary btn-sm hotel-item__view-detail w-100 d-flex justify-content-center align-items-center"
                                    href="{{route('view-apartment', ['id' => $hashids->encode($apartment->id)])}}"
                    ...
                    @empty
                    <div class="col-12">
                        <p class="text-center">No apartments found with coordinates.</p>
                    </div>
                    @endforelse
                </div>
...
<script>
    function initMap() {
        ...
        // Apartment data from Laravel
        const apartments = @json($apartments);

        // Add markers for each apartment
        apartments.forEach(apartment => {
            const marker = new google.maps.Marker({
                position: { lat: parseFloat(apartment.lat), lng: parseFloat(apartment.long) },
                map: map,
            ...
            const infoWindow = new google.maps.InfoWindow({
                content: `
                    <div style="padding-left: 10px; padding-right: 10px; max-width: 250px;">
                        ${apartment.cover_photo ? `<img src="storage/${apartment.cover_photo}" alt="${apartment.title}" style="width: 50px; height: 50px; object-fit: cover; border-radius: 8px; margin-bottom: 8px;">` : ''}
                        <h6 style="margin: 0 0 5px 0; font-weight: 600;">${apartment.title}</h6>
                        <p style="margin: 0 0 5px 0; color: #666; font-size: 14px;">
                            <i class="fas fa-map-marker-alt"></i> ${apartment.location}
                        </p>
                        <p style="margin: 0 0 5px 0; color: #2e8b57; font-weight: 600;">
                            USD ${parseFloat(apartment.base_price).toFixed(2)}/night
                        </p>
                        ${apartment.category ? `<span style="background: #2e8b57; color: white; padding: 2px 8px; border-radius: 3px; font-size: 12px;">${apartment.category.category}</span>` : ''}
                    </div>
                `
            });
            ...
            // Add click listener to apartment cards to center map on marker
            const apartmentCard = document.querySelector(`[data-id="${apartment.id}"]`);
            if (apartmentCard) {
